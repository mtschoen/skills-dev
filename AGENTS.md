...
CI reads the fork revision from `.aislop/fork-commit` (one lowercase 40-hex SHA). The `aislop` job validates it before exporting it, clones the fork into `$RUNNER_TEMP/aislop`, and runs `npx -y pnpm@10.28.0 --dir "$RUNNER_TEMP/aislop" install --frozen-lockfile`. The fork's `prepare` script builds `dist/cli.js`, which CI runs directly with `ci .`; no tarball or global installation is involved. Gitea runs clone `schoen/aislop` from the current server; GitHub runs use the `mtschoen/aislop` mirror. Bump the pin deliberately and confirm that the commit is available on both forges, includes the pnpm lockfile, and passes the score-100 gate. Local global-install instructions below are unchanged.
