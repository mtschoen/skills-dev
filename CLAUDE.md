...
- Per-submodule remote convention: `origin` → GitHub (SSH, **primary**), `gitea` → Gitea (SSH, **mirror**). **Always push to both when both are configured** so the hosts don't drift. (Some older skill repos instead carry a redundant `github` remote duplicating `origin`/GitHub and have no `gitea` remote — harmless; `push-all` pushes every configured remote name and skips the rest, so retrofitting them is just adding a `gitea` remote.) On top of whatever remote names a clone has, `scripts/setup_remotes.py` sets **dual-host push URLs on `origin`** (gitea + github, derived from the repo basename) so even a plain `git push origin` updates both hosts — run it once per clone; it is what prevents a repeat of the 2026-07 wrap/skills-dev divergence, where each host silently accumulated commits the other never saw.
...
**Reinstalling a skill mid-session does not affect the running session.** The harness loads skills at session start, so `install-skills.sh` followed by invoking the skill in the same session hands back the *previous* SKILL.md text. Observed 2026-08-04: wrap was reinstalled, verified correct on disk, then invoked - and the pre-change version loaded. Verify with `grep` against the installed file rather than trusting the invocation, and treat any same-session skill test as testing the old version. This is the same trap as auditing a skill you forgot to install: you get a stale artifact and no error.
...
**Fresh-clone setup:** run `git config submodule.recurse true` and `python3 scripts/setup_remotes.py` once per clone (neither can be committed — `.git/config` is per-clone; the script gives every repo dual-host push URLs on `origin`). Without it, pulling the umbrella advances the recorded submodule pointers but leaves local checkouts behind, producing the recurring `M <submodule> (new commits)` drift. See README "Cloning" for the trade-off (pulls then detach submodule HEADs; pull-all re-attaches to main).
